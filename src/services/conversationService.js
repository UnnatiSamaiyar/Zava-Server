...
export const findConversation = async (sender_id, receiver_id) => {
  ...
  if (receiver_id) {
    ...
    // Group chat: fetch all groups sender is part of
    convos = await ConversationModel.find({
      isGroup: true,
      users: { $elemMatch: { $eq: sender_id } },
    })
...
export const createConversation = async (convoData) => {
  ...
  if (convoData.isGroup) {
    if (!convoData.admin) {
      throw createHttpError.BadRequest("Group admin is required");
    }
    if (!convoData.users || convoData.users.length < 2) {
      throw createHttpError.BadRequest(
        "Group must have at least 2 members (including admin)"
      );
    }
  } else {
